    ...
    <section className="px-4 pt-4 mb-28">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <label className='flex flex-col mt-4 text-sm text-gray-500'>
          Employee name
          <input className='h-10 mt-1 px-2 text-gray-900 rounded bg-gray-100 border border-gray-300' placeholder='Enter employee name' type="text" id="name" {...register("name", {required: true})} />
          {errors.name && <span className="text-sm text-red-600">This field is required</span>}
        </label>
        <label className='flex flex-col mt-4 text-sm text-gray-500'>
          Position
          <input className='h-10 mt-1 px-2 text-gray-900 rounded bg-gray-100 border border-gray-300' placeholder='Enter position' type="text" id="position" {...register("position", {required: true})} />
